...
import com.example.myapplication.views.AVLoadingIndicatorView;
...
public abstract class BaseActivity extends ASActivity {
    ...
    protected void requestHint() {
        HintRequest hintRequest = new HintRequest.Builder()
                ...
                .build();
        CredentialsClient credentialsClient = Credentials
                .getClient(this);
        PendingIntent intent = credentialsClient.getHintPickerIntent(
                hintRequest);
        try {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 6) {
            if (resultCode == RESULT_OK) {
                ...
                String mobileno = credential.getId();//  <-- will need to process phone number string
            }
        }
    }
